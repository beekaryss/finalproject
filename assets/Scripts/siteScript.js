...
function submitActionUserBuy(typeReq, camName) {
    let authUser = JSON.parse(localStorage.getItem('authUser') || "[]");
    ...
    if(typeReq == 1) {
        swal("Are you sure?", {
            ...
            if(value == "Yes"){
                ...
                Request.push({user: authUser.eMail, type: "Buy", camName: camName, ans: ""});
        ...
        swal("Are you sure?", {
            ...
            if(value == "Yes"){
                ...
                let Request = JSON.parse(localStorage.getItem('Request') || "[]");
                Request.push({user: authUser.eMail, type: "Test", camName: camName, ans: ""});
                localStorage.setItem('Request', JSON.stringify(Request));
            }
        });
    }
    ...
    for(let i of Request){
        if(i.user == authUser.eMail) {
            $('#tableForAllResultsProfile tbody').append('<tr id="trForRemoveRequestList"><td>' + t + '</td>' +
                '<td>' + i.type + '</td><td>' + i.camName + '</td><td><textarea cols="85" rows="3">' + i.ans + '</textarea>' +
